    ...
    void SetupRequest(FCurlHttpRequest* Request) {
        if (FCurlHttpRequest::SetURLIdx == 0) {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                if (CheckBytes<0x8D, 0x91>(GetFunc, i)) {
                    URLOffset = *(uint32_t*)(__int64(GetFunc) + i + 2);
                    break;
                }
            }

            if (URLOffset == 0) goto def;
            for (int64_t i = 1; i < ((__int64(FCurlHttpRequest::ProcessRequestVT) - __int64(Request->VTable)) / 8) /* search radius */; i++) {
                auto func = Request->VTable[i];
                for (int j = 0; j < 0x20; j++) {
                    if (CheckBytes<0x48, 0x81, 0xC1>(func, j)) {
                        if (*(uint32_t*)(__int64(func) + j + 3) == URLOffset) {
                            printf("SetURLIdx: %lld", i);
                            FCurlHttpRequest::SetURLIdx = i;
                            return;
                        }
                    }
                    if (CheckBytes<0x81, 0xE9>(func, j))
                    {
                        if (-*(int32_t*)(__int64(func) + j + 2) == URLOffset)
                        {
                            printf("SetURLIdx: %lld", i);
                            FCurlHttpRequest::SetURLIdx = i;
                            return;
