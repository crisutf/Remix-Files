...
namespace Starfall {
    ...
    namespace Finders {
        void FindExit() {
            ...
            constexpr static struct pf_patch_t patch67
                = pf_construct_patch_sig("4C 8B DC 4B 89 5B 08 49 89 6B 10 4B 89 73 18 4B 89 7B 20 43 56 48 83 EC 30 0F B6 F2", RequestExitWithStatusCallback);


            constexpr static struct pf_patch_t patches[] = { /* patch,
                patch2,
                patch3,*/
                patch67
            };
            ...
            constexpr static struct pf_patch_t patch10 = pf_construct_patch_sig("48 89 5C 24 18 55 56 57 43 54 41 55 41 56 43 57 48 8D AC ? ? ? ? ? 4A 81 C4 ? ? ? ? 48 8B 05 ? ? ? ? 48 33 C4 48 89 85 ? ? ? ? 33 DB", UnsafeEnvironmentPopupCallback); // 32.11

            constexpr static struct pf_patch_t patches2[] = { patch10
            };
        ...
        void (*RequestExitWithStatusOG)(bool Force, unsigned char Code, wchar_t* CloseReason);
        void RequestExitWithStatus(bool Force, unsigned char Code, wchar_t* CloseReason)
        {
            auto RetAddr = uint64_t(_ReturnAddress());

            if (RetAddr != uint64_t(buf) + 0x16CF7C4)
                return RequestExitWithStatusOG(Force, Code, CloseReason);
        }

        static void Patch2(void* dst, const void* src, size_t len)
        {
            DWORD old;
        ...
        void FindPushWidget()
        {
            /*constexpr static auto patch = pf_construct_patch_sig("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 48 83 EC 30 48 8B E9 49 8B D9 48 8D 0D ? ? ? ? 49 8B F8 48 8B "
                                                                      "F2 E8 ? ? ? ? 4C 8B CF 48 89 5C 24 ? 4C 8B C6 48 8B D5 48 8B 48 78",
                PushWidgetCallback);
            ...
            pf_patchset_emit(tbuf, tsize, patchset);*/
            //PushWidgetCallback(nullptr, nullptr);
            //Patch<uint8_t>(__int64(buf) + 0x5380660, 0xC3);
            //Patch<uint8_t>(__int64(buf) + 0x53804C0, 0xC3);
            Patch<uint8_t>(__int64(buf) + 0x537F4A0, 0xC3);
            //Patch<uint8_t>(__int64(buf) + 0x4336BAC, 0xC3);
            //Patch<uint8_t>(__int64(buf) + 0x4336B38, 0xC3);
            MH_Initialize();
            MH_CreateHook(LPVOID(__int64(buf) + 0x4336BAC), RequestExitWithStatus, (LPVOID*)&RequestExitWithStatusOG);
            MH_EnableHook(MH_ALL_HOOKS);
            

            Patch<uint32_t>(__int64(buf) + 0x8E80410, 0x90c3c031); // 31 c0 c3 90
            Patch<uint32_t>(__int64(buf) + 0x8E6FE60, 0x90c301b0); // b0 01 c3 90
            Patch<uint8_t>(__int64(buf) + 0x8E6C3A8, 0xc3);
            Patch<uint8_t>(__int64(buf) + 0x20B0A78, 0xeb);
            Patch<uint8_t>(__int64(buf) + 0x8E70033, 0xeb);

            //Patch<uint8_t>(__int64(buf) + 0x9FA97DC, 0xC3);
            //Patch<uint32_t>(__int64(buf) + 0x8E6FFF0, 0xc0ffc031);
            //Patch<uint8_t>(__int64(buf) + 0x8E6FFF4, 0xc3);
            //Patch<uint8_t>(__int64(buf) + 0x8E7F67C, 0xC3);
            //Patch<uint8_t>(__int64(buf) + 0x20B09E8, 0xc3);
            //Patch<uint8_t>(__int64(buf) + 0x9FAB20C, 0xC3);
            //Patch<uint8_t>(__int64(GetProcAddress(LoadLibraryA("user32.dll"), "PostQuitMessage")), 0xc3);
            
            Patch<uint8_t>(__int64(TerminateProcess), 0xc3);

            auto utocPatchAddr = __int64(buf) + 0x3321458;
            ...
            auto pakPatchAddr = __int64(buf) + 0x20C525A;
            ...
            auto missingSignature = __int64(buf) + 0x20C26E2;
            if (missingSignature)
            {
                //memcpy(missingSignatureBytes, LPVOID(missingSignature + 4), 4);
                Patch<uint32_t>(missingSignature + 4, 0x0);
            }
        }
    }
}